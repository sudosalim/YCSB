...
public class Couchbase3Client extends DB {
  ...
  private Status soeUpdateKv(Generator gen)  {
    Collection collection = bucket.defaultCollection();
    if (useDurabilityLevels) {
      collection.reactive().replace(gen.getCustomerIdWithDistribution(),
            JsonObject.fromJson(gen.getPredicate().getValueA()),
            replaceOptions()
                  .expiry(Duration.ofHours(documentExpiry))
                  .durability(durabilityLevel));
    ...
    QueryResult queryResult = cluster.query(
          ...
          queryOptions()
                .parameters(JsonArray.from(
                      ...
                      gen.getPredicate().getNestedPredicateA().getValueA()))
                .adhoc(adhoc)
                .maxParallelism(maxParallelism)
    );
  ...
  private Status soeReadKv(HashMap<String, ByteIterator> result, Generator gen)
        throws Exception {
    Collection collection = bucket.defaultCollection();
    GetResult getResult = collection
          .get(gen.getCustomerIdWithDistribution(),
    ...
    soeDecode(getResult.contentAs(String.class), null, result);
    return Status.OK;
  }
  ...
  private Status soeReadN1ql(HashMap<String, ByteIterator> result, Generator gen)
    ...
    JsonObject row;
    try {
      row = queryResult.rowsAsObject().get(0);
  ...
  private Status soeScanN1ql(final Vector<HashMap<String, ByteIterator>> result, Generator gen) {
    ...
    for (JsonObject row : queryResult.rowsAsObject()) {
      ...
      soeDecode(row.toString(), null, tuple);
      result.add(tuple);
    }
    return Status.OK;
  }
